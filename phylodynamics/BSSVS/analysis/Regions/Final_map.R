...
###############################################################################
# 4. Join zone info to the new map data
###############################################################################
europe_data <- europe_with_russia %>%
  left_join(country_data, by = c("name" = "Country"))

###############################################################################
# 5. Manual color palette for each zone, then lighten
###############################################################################
region_pal <- c(
  "Russia"         = "#F4D6A0", 
...
region_pal_light <- sapply(region_pal, lighten_color, factor = 0)

###############################################################################
# 6. Approximate coordinates for zone “centers”
###############################################################################
...
###############################################################################
# 7. Example arrow data + BF categories
###############################################################################
arrows <- data.frame(
