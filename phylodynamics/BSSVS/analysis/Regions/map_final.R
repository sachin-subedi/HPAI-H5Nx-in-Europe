...
lighten_color <- function(color, factor = 0.6) {  # Adjust factor to control lightness
  col <- col2rgb(color)
...
###############################################################################
# 4. *Shrink* the width of that Russia subset via an affine transform
###############################################################################
rus_geom  <- st_geometry(russia_subset)
rus_union <- st_union(rus_geom)
rus_cent  <- st_centroid(rus_union)
cx <- st_coordinates(rus_cent)[1]
cy <- st_coordinates(rus_cent)[2]

# Shift centroid to (0,0)
rus_geom <- rus_geom - c(cx, cy)
...
rus_geom <- rus_geom %*% scale_mat

# Shift back
rus_geom <- rus_geom + c(cx, cy)

st_geometry(russia_subset) <- rus_geom

# Combine the clipped & transformed Russia with the rest of Europe
...
###############################################################################
# 6. Manual color palette for each zone, then lighten
###############################################################################
region_pal <- c(
...
# Apply your lighten_color function to each color
region_pal_light <- sapply(region_pal, lighten_color, factor = 0.6)
...
ggplot(data = europe_data) +
  ...
  scale_fill_manual(values = region_pal_light, name = "Regions") +
